...
///*********************************************************/			
				...
				PIC_display1(gImage_4);     //EPD_picture
				lut_DU();
				EPD_refresh();	
				KEY_Scan();		
	
			 
				PIC_display1(gImage_5);     //EPD_picture
				lut_DU();
				EPD_refresh();		
				KEY_Scan();		
				
				PIC_display1(gImage_6);     //EPD_picture
				lut_DU();
				EPD_refresh();	
				KEY_Scan();		
				
				
				PIC_display1(gImage_7);     //EPD_picture
				lut_DU();
				EPD_refresh();	
				KEY_Scan();	
					
				
				PIC_display1(gImage_8);     //EPD_picture
				lut_DU();
				EPD_refresh();		
				KEY_Scan();	
				
				PIC_display1(gImage_9);     //EPD_picture
				lut_DU();
				EPD_refresh();	
				KEY_Scan();	
...
LUT_Flag=0;
	...
	EPD_W21_WriteDATA(0xF3);			// RES1 RES0 REG KW/R     UD    SHL   SHD_N  RST_N	
...
void GPIO_Configuration_Sleep(void)
{
  GPIO_InitTypeDef  GPIO_InitStructure;
	...
	// BUSY--->PE13
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_13|GPIO_Pin_11;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AIN;	//Pull up input
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
 	GPIO_Init(GPIOE, &GPIO_InitStructure);				//Initialize GPIO
